...
import { extractToken, AuthErrors, UserWithId } from '../../lib/auth';
...
export const UserCtx = createParamDecorator(
  (data: any, ctx: ExecutionContext) => {
    ...
    if (!guards.some((guard) => guard === AuthGuard)) {
      throw new Error(
        `Route '${handler.name}' uses @UserCtx decorator but missing AuthGuard.\n` +
          `Please add @UseGuards(AuthGuard) to enable user extraction.`,
      );
    }

    const request = ctx.switchToHttp().getRequest();
    const user = request.user as UserWithId | undefined;
